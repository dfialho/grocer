...
class ContinenteProcessor(override val watchDirectory: Path) : ReceiptFileProcessor {
    ...
    override fun onReceiptFile(receiptFile: Path) {
        ...
        println(ContinenteReceipt(id, date, items))
    }
    ...
    private fun readItems(iterator: ListIterator<String>): MutableList<ContinenteItem> {
        while (iterator.hasNext()) {
            ...
            if (line.contains("DESCRICAO VALOR")) {
                logger.debug { "Reached the start of the items list" }
                break
            }
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (line.startsWith("TOTAL A PAGAR")) {
                logger.debug { "Reached the end of the items list" }
                break
            }

            if (line.endsWith(":")) {
                logger.debug { "Category line: $line" }
                ...
                logger.debug { "Item line: $line" }
                val lineWithoutTaxTag = line.substringAfter(") ")
                logger.info { "Item '$lineWithoutTaxTag'" }
                ...
                val item = if (tokens.last().matches(Regex("\\d+,\\d\\d"))) {
                    ...
                    logger.debug { "Amount is in the next line" }
    ...
    private fun readDate(iterator: ListIterator<String>): LocalDate {
        ...
        while (iterator.hasNext()) {
            ...
            if (line.startsWith("Nro: ")) {
                logger.debug { "Reached line with date" }
